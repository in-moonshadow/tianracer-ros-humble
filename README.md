...
`tianracer_description` injects a default for `TIANRACER_BASE` (see `env-hooks/99.tianracer.dsv.in`);
`TIANBOT_NAME` is set by you in the environment (unset = no namespace):

| Variable | Values | Effect |
|---|---|---|
| `TIANRACER_BASE` | `compact` (default), `standard`, `fullsize` | Vehicle model / wheelbase |
