...
> ⚠️ 出生位姿必须实测，不要凭地图猜：z 要高于地板顶面（否则车生成在地板下），
...
`tianracer_description` 会注入以下默认值（见 `tianracer_description/env-hooks/99.tianracer.dsv.in`）：

| 变量 | 取值 | 作用 |
|---|---|---|
| `TIANRACER_BASE` | `compact`（默认）/ `standard` / `fullsize` | 车型：决定 TF 分支与轴距 |
| `TIANRACER_LIDAR` | `richbeam`（默认）/ `rplidar_a1` / `velodyne` / `rslidar` | 雷达型号：决定激光/IMU 的静态变换 |
...
这不影响跑仿真竞速——它们只用于「改进参数」这件事。若你需要做参数调优：

- 参数档（`params/*.yaml`）里保留了每个非默认值的**定稿依据**（实测数据、被否证的尝试、
  失败点位等），可据此理解为什么是这些值；
- 自行搭建测试链时，建议至少记录：每轮完赛情况与用时、车的位置轨迹、卡住时的位置与
  当时的控制指令——只有这样才分得清「配置问题」和「随机性」。
