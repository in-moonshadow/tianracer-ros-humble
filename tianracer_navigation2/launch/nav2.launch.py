...
# 注入后的 params 写到这里（固定名 + 覆盖写）：不用 mkstemp，避免反复启动后残留一堆
# 随机命名的临时文件。
#
# 目录放在**当前用户私有**的运行时目录，而不是全局可写的 /tmp：/tmp 下固定文件名意味着
# 任何本地用户可以预先抢占该路径（符号链接/篡改），写入方却毫不知情。$XDG_RUNTIME_DIR
# 由系统按用户创建、权限 0700，是最合适的位置；先按目录是否真实存在来判定，不只看环境变量
# （拿不到该变量时退化到 /tmp，避免直接写进不存在的目录）。
_XDG_RUNTIME = os.environ.get('XDG_RUNTIME_DIR', '')
_RUNTIME_BASE = _XDG_RUNTIME if (_XDG_RUNTIME and os.path.isdir(_XDG_RUNTIME)) else '/tmp'
SPAWN_PARAMS_DIR = os.path.join(_RUNTIME_BASE, 'tianracer_nav2_params')
...
# （此时 nav2.log 里 `Running backup` 计数恒为 0、卡死都伴随 `spin failed`，即此回退的指纹）。这里由父 launch 注入路径，是 Nav2 官方示例认可的方式
...
def _params_with_spawn(src_params, world, out_dir=SPAWN_PARAMS_DIR):
    ...
    初始位姿错了无法自愈。二者各写一份硬编码值，换赛道时漏改其中一处即出错。
