...
def generate_launch_description():
    ...
    # 重要：不要设置 NVIDIA EGL 环境变量（__GLX_VENDOR_LIBRARY_NAME 等）——会导致
    ...
    # 传感器只有在文件方式生成时才被 gz-sim Sensors 系统正确挂载。
    def spawn_robot(context):
        ...
        # namespace 会被拼进下面的 shell 命令，故必须是安全字符集，否则可注入任意命令。
        # 用白名单而非 shlex.quote：ns 本身嵌在双引号里（xacro 的 prefix:='...' 形式），
        # shlex 产生的单引号会破坏该语法。命名空间本就只应是 [A-Za-z0-9_/]，拒绝其余字符。
        if not re.fullmatch(r'[A-Za-z0-9_/]*', ns):
            raise RuntimeError(
                'namespace 含非法字符（仅允许字母/数字/下划线/斜杠）: %r' % ns)

        # 放在当前用户私有的运行时目录而非全局可写的 /tmp：固定文件名若落 /tmp，
        # 任何本地用户都能预先抢占该路径（符号链接/篡改）。
        _rt = os.environ.get('XDG_RUNTIME_DIR', '')
        urdf_dir = _rt if (_rt and os.path.isdir(_rt)) else '/tmp'
        urdf_path = os.path.join(urdf_dir, 'tianracer_robot.urdf')

        # 三个变量（xacro 路径、namespace、输出路径）一律用 shlex.quote 单引号引用：
        # 路径可能含空格，ns 已过白名单，输出路径由上面的目录推导而来。
        urdf_cmd = ExecuteProcess(
            cmd=['bash', '-c',
                 'xacro %s %s > %s' % (shlex.quote(xacro_file),
                                       shlex.quote("prefix:='%s'" % ns),
                                       shlex.quote(urdf_path))],
            output='screen')
        ...
        # 服务端存在传输版本不匹配，create 服务响应偶发丢失导致超时（ign service 则稳定返回 data:true）。
        ...
        # ign service create 可能读到尚未写完的 urdf。
        return [urdf_cmd,
    ...
    return LaunchDescription([
        ...
        # launch 内启动的 gz server 偶发 100% CPU 空转、create 服务无响应。
        OpaqueFunction(
        ...
        #（ros_gz_sim create 节点用字符串生成模型时传感器不更新，无数据）。
        ...
        Node(
            ...
            arguments=[
                ...
                # 同名 /clock 话题没有任何消息（ign topic -e -t /clock 与
                ...
                # 为什么不用 /world/default/control 的 reset：gz-sim 6 下
