...
本模块把位姿收敛到唯一一张表，三处都从这里取，避免换赛道时只改了其中一处。
...
⚠️ z 必须 ≥ 该赛道地板顶面，否则车生成在地板实体内部、被物理引擎弹到地板【下方】，
   表现为 Gazebo 里看不到车、雷达被埋扫不到墙。地板顶面高度需实际测量确认，
   不要按 DAE 包围盒静态推断——DAE 的 <node> 矩阵读法易错。
"""
...
TRACK_SPAWNS = {
    ...
    # 地板顶面 0.5（实心厚板）。z=0.55 = 顶面 +0.05 余量，轻落后落在板上。
    # 生成点 (0,0) 落在一条 1.03m 宽、南北向走廊正中（西墙 x=-0.638~-0.488、
    ...
    # 地板顶面 0.5。生成点【就是】本赛道路点0 (15.974746,1.153895)（位于地板上），
    # 故朝向可直接取路点0 朝向 -1.6300(-93.4°)；该朝向正前方 8m 无墙 ✅。
    # 判据：只有「出生点 = 路点0」时才能借用路点0 朝向（见 test_indoor 的反例）。
    'racetrack_1': (15.974746, 1.153895, 0.55, -1.6300),

    # 地板顶面 0.818（其 DAE 名义厚 2.0，实际碰撞面 0.818）。
    ...
    # 与独立测量一致。起跑线中心 (0.112,-0.001)，与 (0,0) 同属一格自由空间
