...
class RaceStateMachine(Node):
    ...
    # spin 节流参数，取值依据见 _spin_drain 的实测数据
    ...
    def __init__(self, filename, repeat=True):
        ...
        # 状态机计数：用于核对「到点次数 vs 下发目标次数」。
        # 实测曾出现 close to goal 424 次而 Move to / Goal accepted 仅各 3 次的异常，
        # 仅靠日志无法判定，故把各分支的进入次数显式计数并周期输出。
    ...
    def _spin_drain(self):
        ...
        话题约 960Hz，于是循环以消息到达速率空转（实测 1000 次/秒、31.6% 单核；
        纯高频话题下可达 9600 次/秒、114%）。
        ...
        实测（960Hz /clock + 50Hz 低频话题，各 5s）：
            busy spin_once(0.05)   1000 次/秒   低频投递 50.8Hz   31.6% 单核
            本方案                  205 次/秒   低频投递 49.6Hz    8.0% 单核
        排空超时不能取得更小：取 0.2ms 时单轮只处理约一个回调，低频话题被 /clock
        挤到 25.1Hz（掉一半）——故取 1ms。
        """
    ...
    def _wait_for_acceptance(self):
        ...
        就被误判超时放弃（实测 accept_timeout=1，发车线第一个路点被跳过）。
        """
