...
class Judger(Node):
    ...
    # 约一个发布周期，AMCL 的 MessageFilter 会以「外推到未来」丢弃它。
    ...
    def __init__(self):
        ...
        # 默认取官方值 30.0。本仿真实时率≈0.95（仿真时间≈墙钟），车速偏低时单门间隔
        # 可能超过该阈值、在首个门前就终止比赛；调试期可临时调大，传 0.0 表示关闭该判据。
    ...
    def _cal_racing_score(self):
        ...
        scores。故比赛中界面显示的是纯门分，提前终止时也是纯门分。
        """
    ...
    def _reset_world(self):
        ...
        gz-sim 6 下 ControlWorld 的 reset（model_only 与 all 都试过）会返回 success
    ...
    def _publish_init_pose(self):
        ...
        odom->base_footprint 超前约一个发布周期，MessageFilter 查不到
        对应时刻的 TF 就把整条消息丢掉，AMCL 定位不更新（后果：规划器出不了路径、
    ...
    def _cancel_nav_goals(self):
        ...
        而执行器正在跑这个回调，重入 spin 拿不到任何响应——会一直等到超时且
        result() 为 None（同一调用放在回调外即刻返回），于是 reset 表面成功、
